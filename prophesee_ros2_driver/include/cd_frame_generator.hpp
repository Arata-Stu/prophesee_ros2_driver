...
public:
    ...
    ///
    /// @param msg : event buffer message (ROS2 SharedPtr)
    void add_events(const prophesee_event_msgs::msg::EventArray::SharedPtr &msg);
    ...
    const rclcpp::Time &get_last_ros_timestamp() const;

    /// \brief Converts ROS time into Metavision timestamp
    ///
    /// \return Timestamp in microseconds
    inline Metavision::timestamp ros_timestamp_in_us(const rclcpp::Time &ts) const {
        return static_cast<Metavision::timestamp>(ts.nanoseconds() / 1000.0);
...
private:
    ...
    // Received events
    std::vector<prophesee_event_msgs::msg::Event> events_queue_front_;

    // Events to display
    std::vector<prophesee_event_msgs::msg::Event> events_queue_back_;
