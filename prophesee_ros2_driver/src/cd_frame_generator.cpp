...
void CDFrameGenerator::add_events(const prophesee_event_msgs::msg::EventArray::SharedPtr &msg) {
    bool should_process = false;
    {
        std::lock_guard<std::mutex> lock(processing_mutex_);
        if (!msg->events.empty()) {
            events_queue_front_.insert(events_queue_front_.end(), msg->events.begin(), msg->events.end());
            last_ts_ = ros_timestamp_in_us(msg->events.back().ts);
        }
    ...
    // 変更：Node側のクロックを利用する
    if (clock_ptr_)
        last_ros_ts_ = clock_ptr_->now();
    else
        last_ros_ts_ = rclcpp::Clock().now();
}
...
void CDFrameGenerator::set_display_accumulation_time_us(long long display_accumulation_time_us) {
    std::lock_guard<std::mutex> lock(processing_mutex_);
    display_accumulation_time_us_ = static_cast<uint32_t>(display_accumulation_time_us);
}
...
const rclcpp::Time &CDFrameGenerator::get_last_ros_timestamp() const {
    return last_ros_ts_;
}
...
bool CDFrameGenerator::start() {
    ...
    if (thread_.joinable()) { // 既に開始済みなら false
        return false;
    }
